...
const _runCreate = c =>
    new Promise((resolve, reject) => {
        ...
        readline.question(
            getQuestion("What's your project Name? (no spaces, folder based on ID will be created in this directory)"),
            v => {
                // console.log(`Hi ${v}!`);
                data.projectName = v;
                    ...
                    readline.question(getQuestion(`What\'s your App ID? (${data.appID}) will be used by default`), v => {
                        ...
                        logSuccess(
                            `Your project is ready! navigate to project ${chalk.white(
                                `cd ${data.projectName}`
                            )} and run ${chalk.white('rnv run -p web')} to see magic happen!`
                        );
...
const _checkAndCreatePlatforms = (c, platform) =>
    new Promise((resolve, reject) => {
...
const _runPlugins = (c, pluginsPath) =>
    new Promise((resolve, reject) => {
        logTask('_runPlugins');
