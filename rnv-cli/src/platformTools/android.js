...
const configureProject = (c, platform) => new Promise((resolve, reject) => {
    ...
    const privateConfig = _getPrivateConfig(c, platform);

    if (privateConfig) {
        const keystorePath = privateConfig[platform].storeFile;
        let keystorePathFull;
        if (keystorePath) {
            if (keystorePath.startsWith('./')) {
                keystorePathFull = path.join(privateConfig.parentFolder, keystorePath);
            } else {
...
keyAlias=${privateConfig[platform].keyAlias}
storePassword=${privateConfig[platform].storePassword}
keyPassword=${privateConfig[platform].keyPassword}`);
