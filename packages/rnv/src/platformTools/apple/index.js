...
const runPod = async (c, platform) => {
    ...
    const podsRequired =
        c.program.updatePods || (await checkIfPodsIsRequired(c));
...
export const runXcodeProject = async c => {
    ...
    if (p) {
        ...
        if (bundleAssets) {
            return packageBundleForXcode(c, c.platform, bundleIsDev).then(() =>
                _checkLockAndExec(c, appPath, scheme, runScheme, p)
            );
        }
        return _checkLockAndExec(c, appPath, scheme, runScheme, p);
...
const _checkLockAndExec = async (c, appPath, scheme, runScheme, p) => {
  ...
  You can find correct teamID in the URL of your apple developer account: ${chalk.white(
      'https://developer.apple.com/account/#/overview/YOUR-TEAM-ID'
                ...
                const { confirm } = await inquirer.prompt({
                    name: 'confirm',
                    message: `Type in your Apple Team ID to be used (will be saved to ${
                        c.paths.appConfig?.config
                    })`,
                    type: 'input'
                });
...
const _handleProvisioningIssues = async (c, e, msg) => {
    ...
    const fixCommand = `rnv crypto updateProfile -p ${c.platform} -s ${
        c.runtime.scheme
    }`;
...
const _setAutomaticSigning = async c => {
    ...
    const scheme =
        c.files.appConfig?.config?.platforms?.[c.platform]?.buildSchemes?.[
            c.runtime.scheme
        ];
    if (scheme) {
...
const composeXcodeArgsFromCLI = string => {
    ...
    const unescapedValues = keysAndValues.map(s =>
        s
            .replace(/\'/g, '')
            .replace(/"/g, '')
            .replace(/\\/g, '')
    ); // removes all quotes or backslashes
