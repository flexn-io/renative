...
export const parseAppBuildGradleSync = (c, platform) => {
    ...
    // c.files.private.appConfig.configPrivate = _getPrivateConfig(c, platform);

    if (c.files.private.appConfig.configPrivate && c.files.private.appConfig.configPrivate[platform]) {
        const keystorePath = c.files.private.appConfig.configPrivate[platform].storeFile;
        let keystorePathFull;
        if (keystorePath) {
            if (keystorePath.startsWith('./')) {
                keystorePathFull = path.join(c.paths.private.appConfig.dir, keystorePath);
            } else {
...
keyAlias=${c.files.private.appConfig.configPrivate[platform].keyAlias}
storePassword=${c.files.private.appConfig.configPrivate[platform].storePassword}
keyPassword=${c.files.private.appConfig.configPrivate[platform].keyPassword}`);
...
//     c.paths.private.appConfig.dir = privateConfigPath.replace('/config.private.json', '');
//     if (fs.existsSync(privateConfigPath)) {
//         try {
//             const output = JSON.parse(fs.readFileSync(privateConfigPath));
//             output.parentFolder = c.paths.private.appConfig.dir;
