...
export const parseAppBuildGradleSync = (c, platform) => {
    ...
    if (
        ...
        const keystorePath =
            c.files.workspace.appConfig.configPrivate[platform].storeFile;
...
keyPassword=${c.files.workspace.appConfig.configPrivate[platform].keyPassword}`
    ...
    const debugBuildTypes =
        pluginConfig?.platforms[platform]?.gradle?.buildTypes?.debug ?? [];
    const releaseBuildTypes =
        pluginConfig?.platforms[platform]?.gradle?.buildTypes?.release ?? [];
    ...
    const isMultiApk =
        getConfigProp(c, platform, 'multipleAPKs', false) === true;
