...
const _generateWebpackConfigs = (c, platform) => {
    ...
    const analyzer =
        getConfigProp(c, platform, 'analyzer') || c.program.analyzer;
...
const buildWeb = (c, platform) =>
    new Promise((resolve, reject) => {
        ...
        executeAsync(
            c,
            `npx cross-env PLATFORM=${platform} NODE_ENV=production ${debugVariables} node ${wbp} -p --config ./platformBuilds/${
                c.runtime.appId
            }_${platform}/webpack.config.js`
        )
            .then(() => {
                logSuccess(
...
const _runWebBrowser = (c, platform, devServerHost, port, alreadyStarted) =>
    new Promise(resolve => {
        logTask(
            `_runWebBrowser:${platform}:${devServerHost}:${port}:${
                c.runtime.shouldOpenBrowser
            }`
        );
