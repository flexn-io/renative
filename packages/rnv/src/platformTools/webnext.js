...
const configureNextProject = (c, platform) => new Promise((resolve, reject) => {
...
const deployNext = (c, platform) => new Promise((resolve, reject) => {
    logTask(`deployWeb:${platform}`);
    selectWebToolAndDeploy(c, platform).then(resolve).catch(reject);
});

export { buildNext, runNext, configureNextProject, runNextDevServer, deployNext };
