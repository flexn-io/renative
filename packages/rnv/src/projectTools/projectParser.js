...
export const checkAndCreateProjectPackage = c =>
    new Promise(resolve => {
        ...
        if (!fs.existsSync(c.paths.project.package)) {
            ...
            const packageName =
                c.files.project.config.projectName ||
                c.paths.project.dir.split('/').pop();
            const version =
                c.files.project.config.defaults?.package?.version || '0.1.0';
            const templateName =
                c.files.project.config.defaults?.template ||
...
export const copyRuntimeAssets = c =>
    ...
    if (c.buildConfig) {
        ...
        if (fs.existsSync(c.paths.project.projectConfig.fontsDir)) {
            fs.readdirSync(c.paths.project.projectConfig.fontsDir).forEach(
                font => {
                    if (callback)
                        callback(font, c.paths.project.projectConfig.fontsDir);
                }
            );
        }
        // FONTS - APP CONFIG
...
export const copyAssetsFolder = async (c, platform, customFn) => {
    ...
    if (c.paths.appConfig.dirs) {
        const hasAssetFolder = c.paths.appConfig.dirs.filter(v =>
            fs.existsSync(path.join(v, `assets/${platform}`))
        ).length;
        if (!hasAssetFolder) {
...
export const upgradeProjectDependencies = (c, version) => {
    ...
    if (c.files.project.config?.templates?.[thw]?.version)
        c.files.project.config.templates[thw].version = version;
    if (c.files.project.config?.templates?.[tb]?.version)
        c.files.project.config.templates[tb].version = version;
...
export const configureNodeModules = c =>
    new Promise((resolve, reject) => {
