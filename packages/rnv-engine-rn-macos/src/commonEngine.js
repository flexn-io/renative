...
const { TASK_START, RNM_CLI_CONFIG_NAME } = Constants;
...
export const configureMetroConfigs = async (c) => {
    ...
    // Check rn-cli-config
    if (!fsExistsSync(c.paths.project.rnmCliConfig)) {
        logInfo(
            `Your rn-cli config file ${chalk().white(
                c.paths.project.rnmCliConfig
            )} is missing! INSTALLING...DONE`
        );
        copyFileSync(
            path.join(c.paths.rnv.projectTemplate.dir, RNM_CLI_CONFIG_NAME),
            c.paths.project.rnmCliConfig
        );
    }
};
