...
const nextServerlessLoader: loader.Loader = function() {
  ...
  const dynamicRouteImports = pageIsDynamicRoute
    ? `
    const { getRouteMatcher } = require('@rnv/next/dist/next-server/lib/router/utils/route-matcher');
      const { getRouteRegex } = require('@rnv/next/dist/next-server/lib/router/utils/route-regex');
  `
    : ''
  ...
  const rewriteImports = `
    ...
    const { pathToRegexp, default: pathMatch } = require('@rnv/next/dist/next-server/server/lib/path-match')
  `

  const handleRewrites = `
    const getCustomRouteMatcher = pathMatch(true)
    const {prepareDestination} = require('@rnv/next/dist/next-server/server/router')
  ...
  if (page.match(API_ROUTE)) {
    return `
      ...
      const { apiResolver } = require('@rnv/next/dist/next-server/server/api-utils')
      ${rewriteImports}
    ...
    const {renderToHTML} = require('@rnv/next/dist/next-server/server/render');
    const { tryGetPreviewData } = require('@rnv/next/dist/next-server/server/api-utils');
    const {sendHTML} = require('@rnv/next/dist/next-server/server/send-html');
    const {sendPayload} = require('@rnv/next/dist/next-server/server/send-payload');
